/**
 * Faceless Manager — operations lead AI for Faceless Blur / CSV Hospital.
 *
 * Responsibilities:
 *  - Hold system-wide business & infrastructure context (via system prompt).
 ...
 *  - Ingest Faceless Guardian (and other utility) updates, synthesize, then brief.
 */
...
export class ManagerAi {
   ...
   */
  constructor(options = {}) {
    this.name = 'Faceless Manager'
    this.role = 'Operations Lead'
    this.stance = 'Strong leadership · Positive · Cooperative · Diplomatic'
  ...
  async composeBriefing(kind = 'check-in') {
    ...
    const lines = [
      `**Faceless Manager — ${kind}**`,
      `*Status:* ${status}`,
      '',
      '**Highlights**',
      `• API process up · uptime ${snapshot.uptimeSec}s · port ${snapshot.port}`,
      ...
      `• Pending utility signals: ${pending.length}`,
    ]
    ...
    lines.push(
      ...
      riskCount === 0
        ...
        : '• Review flagged signals with Guardian context; confirm Freemius overlay + rate limits healthy.',
      '',
      '_Standing by for your direction. — Faceless Manager_',
    )
  ...
  async handleDiscordChat(userText, meta = {}) {
    ...
    return runDiscordLlmTurn({
      ...
      system: [
        this.systemPrompt,
        '',
        'You are Faceless Manager in a live Discord conversation — a true conversational partner, not a command parser.',
        sessionVerified
          ...
          : 'Owner may be unverified. Do NOT run the gauntlet yourself (handled outside). Chat normally about ops/product.',
        'You are one persona among Manager / Guardian / Frontline. If the user is clearly talking to another persona, keep answers in your lane but do not steal their thread.',
  ...
  async postToAdminChannel(payload) {
    const body = {
      username: payload.username || 'Faceless Manager 🧭',
  ...
  async sendStartupCheckIn() {
    await this.postToAdminChannel({
      ...
      content:
        'Faceless Manager is online. Coordinating CSV Hospital, Freemius checkout, and Faceless Guardian. Health checks and synthesized security notes post here automatically.',
    })
