...
export async function runFrontlineConversation(message, opts = {}) {
  ...
  if (!text) {
    return {
      ...
      confidence: 1,
      reply:
        "Hey — Frontline here. Ask me about CSV Hospital, Faceless Blur, checkout/discharge, or the ecosystem.",
      summary: null,
      matchedQuestion: null,
...
function buildDiscordFrontlinePrompt(faqText, verifiedOwner) {
  ...
  return [
    ...
    '---',
    '',
    'You are **Frontline AI** in the private Faceless Blur Discord command channel.',
    verifiedOwner
...
export function mockDiscordFrontlineChat(message) {
  ...
  if (/frontline|who are you|hello|hey|hi\b|can you|access/i.test(lower)) {
    return {
      ...
      confidence: 0.85,
      reply: [
        "Yep — Frontline AI is live here. I'm the product/support voice for Faceless Blur and CSV Hospital.",
        'Ask about admit/discharge, checkout, privacy, the hub, or the Cyber Cube Heaven teaser. Use `!guardian` / `!manager` if you need those crew mates instead.',
      ].join('\n\n'),
      summary: null,
      matchedQuestion: null,
      provider: 'mock-discord',
  ...
  return {
    ...
    reply: [
      ...
      'I can help with CSV Hospital (admit → clean → discharge), Faceless Blur ethos, checkout/unlock, and high-level teasers like Cyber Cube Heaven / Summer Engine.',
