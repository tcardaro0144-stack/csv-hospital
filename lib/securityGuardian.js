/**
 * Faceless Guardian — core security & monitoring module for Faceless Blur / CSV Hospital.
 *
 ...
 *  - Exposes Express middleware to guard incoming utility requests.
 *
...
export class SecurityGuardian {
   ...
   */
  constructor(options = {}) {
    this.name = 'Faceless Guardian'
    this.stance =
      'Friendly, Vigilant, and Helpful — never gullible (Zero-Trust with Manager)'
  ...
  async sendStartupGreeting() {
    await this.postToDiscord({
      username: 'Faceless Guardian 🛡️',
      kind: 'greeting',
      title: 'Guardian online',
      content:
        "Faceless Guardian is online. Systems are watched — velocity + billing-location shields armed. Alerts post here automatically.",
    })
  }
  ...
  async flagAnomaly(incident) {
    ...
    await this.postToDiscord({
      ...
      kind: 'anomaly',
      title: incident.type,
      content: `⚠️ **Security Notice:** Detected a ${incident.type}.\n*Details:* ${incident.details}\n*Action:* Handled safely according to protocol.`,
    })
  }

  /** Compact status for Faceless Manager health synthesis. */
  getStatusSummary() {
    let activeIps = 0
    let peakRate = 0
    const now = Date.now()
  ...
  async handleDiscordChat(userText, meta = {}) {
    ...
    return runDiscordLlmTurn({
      ...
      system: [
        ...
        'You are chatting live in Discord — a conversational security partner, not a keyword bot.',
        'Answer naturally about security posture, velocity shields, anomalies, and Zero-Trust philosophy.',
        verified
