/**
 * Cyberpunk presence / activity rotation for Faceless Bridge.
...
export const FACELESS_PRESENCE_ROTATION = [
  { type: ActivityType.Listening, name: 'neon grid static // facelessblur.com' },
  { type: ActivityType.Playing, name: 'CSV Hospital night shift' },
  { type: ActivityType.Watching, name: 'autonomous agent mesh' },
  { type: ActivityType.Listening, name: 'cyber cube frequencies' },
  { type: ActivityType.Competing, name: 'zero-trust perimeter checks' },
  { type: ActivityType.Playing, name: 'Summer Engine compile loop' },
 ...
 * @param {{ intervalMs?: number, activities?: typeof FACELESS_PRESENCE_ROTATION }} [opts]
 * @returns {{ stop: () => void, tick: () => Promise<void> }}
 */
export function startDiscordPresenceLoop(client, opts = {}) {
  const activities = opts.activities || FACELESS_PRESENCE_ROTATION
