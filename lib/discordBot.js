/**
 * Two-way Discord bridge for the CSV Hospital command channel.
 *
...
/** Distinct display identities — never reply as the app bot account. */
export const AGENT_PROFILES = {
  manager: {
    key: 'manager',
    webhookName: 'CSVH Agent — Manager',
    username: 'CSV Hospital Manager',
    label: '🧭',
  },
  guardian: {
    key: 'guardian',
    webhookName: 'CSVH Agent — Guardian',
    username: 'CSV Hospital Guardian',
    label: '🛡️',
  },
  frontline: {
    key: 'frontline',
    webhookName: 'CSVH Agent — Frontline',
    username: 'Frontline AI',
    label: '💬',
  },
  bridge: {
    key: 'bridge',
    webhookName: 'CSVH Agent — Bridge',
    username: 'CSV Hospital Bridge',
    label: '📡',
  },
}

const HELP_TEXT = [
  '**CSV Hospital command channel** — conversational AI by default (24/7 Bridge).',
  '',
  ...
  'Scope: CSV Hospital ops/security/support. Agents reply as themselves.',
].join('\n')
...
export function startDiscordBot({ manager, guardian }) {
  installDiscordProcessGuards({ label: 'csv-hospital-discord' })
  ...
  async function ensureAgentWebhook(channel, agentKey, cacheScope = 'cmd') {
    ...
    if (!hook) {
      ...
      hook = await channel.createWebhook({
        ...
        reason: `CSV Hospital agent identity: ${profile.username} (${cacheScope})`,
      })
