 ...
 * Primary source: lib/prompts/csvHospitalKnowledge.js (authoritative).
 * Supplemental: dynamic /knowledge/*.md and *.txt (CSV Hospital docs only).
 * Optional fallback: content/faq_knowledge.json (structured FAQ matching).
 */
...
import { CSV_HOSPITAL_KNOWLEDGE } from './prompts/csvHospitalKnowledge.js'
...
/**
 * Build Frontline system prompt with authoritative CSV Hospital knowledge.
 * Called whenever the AI handles a message so /knowledge docs can supplement
 * without replacing csvHospitalKnowledge.js.
 *
...
export function buildFrontlineSystemPrompt(opts = {}) {
  ...
  const roleLine =
    opts.mode === 'discord'
      ? 'You are Frontline AI in a live Discord conversation for CSV Hospital.'
      : opts.mode === 'triage'
        ? 'You are Frontline AI for the CSV Hospital support desk (structured triage JSON replies).'
        : 'You are Frontline AI, representing CSV Hospital.'
  ...
  const supplemental =
    knowledgeText && !/empty or unavailable/i.test(knowledgeText)
      ? [
          '',
          'Supplemental CSV Hospital documents from /knowledge (must not contradict the authoritative knowledge base above):',
          knowledgeText,
        ]
      : []
  ...
  return [
    ...
    ownerLine,
    '',
    'Authoritative CSV Hospital knowledge base (exclusive product source):',
    CSV_HOSPITAL_KNOWLEDGE,
    ...supplemental,
    opts.extra ? `\n${opts.extra}` : '',
  ]
    .filter(Boolean)
